...
from models.user.user import User, UserRole, PasswordHintType, db
from models.user.user_schema import UserSchema
from schemas.auth_schema import RegisterSchema, LoginSchema, ChangePasswordSchema, PasswordHintSchema
from utils.validators import validate_password, validate_username
from utils.constants import ANIMAL_LIST, ACCESS_TOKEN_EXPIRE_MINUTES, REFRESH_TOKEN_EXPIRE_DAYS
...
def refresh_token():
    ...
    if not refresh_token:
        blp.abort(401, message="No refresh token provided")
    ...
    try:
        ...
        if payload.get('type') != 'refresh':
            blp.abort(401, message="Invalid token type")
        
        user = User.query.get(user_id)
        if not user or not user.is_active:
            blp.abort(401, message="Invalid user")
        ...
        blp.abort(401, message="Refresh token expired")
    except jwt.InvalidTokenError:
        blp.abort(401, message="Invalid refresh token")
