...
def require_auth():
    ...
    if 'user_id' not in session:
        blp.abort(401, message="Authentication required")
...
def create_group(args):
    ...
    try:
        ...
        db.session.rollback()
        blp.abort(400, message="Error creating group")
...
def update_group(args, group_id):
    ...
    if group.created_by != current_user.id:
        blp.abort(403, message="Only the group creator can update this group")
    ...
    try:
        ...
        db.session.rollback()
        blp.abort(400, message="Error updating group")
...
def delete_group(group_id):
    ...
    # Check if current user is the creator
    if group.created_by != current_user.id:
        blp.abort(403, message="Only the group creator can delete this group")
    ...
    try:
        ...
        db.session.rollback()
        blp.abort(400, message="Error deleting group")
...
def join_group(group_id):
    ...
    try:
        ...
        db.session.rollback()
        blp.abort(400, message="Error joining group")
...
def leave_group(group_id):
    ...
    if group.created_by == current_user.id:
        blp.abort(400, message="Group creator cannot leave the group. Transfer ownership or delete the group instead.")
    ...
    try:
        ...
        db.session.rollback()
        blp.abort(400, message="Error leaving group")